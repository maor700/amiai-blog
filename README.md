...
סגנון פשוט בהשראת Medium: לבן, עמודה אחת ברוחב 680px, גוף המאמר ב-Noto Sans Hebrew (כמו באתר הקודם), כותרות וממשק ב-Heebo, ציטוטים עם פס בצד, מצב כהה (מתג בכותרת, נשמר ב-localStorage, אחרת לפי המערכת). כל ה-CSS בקובץ אחד: `src/styles/base.css`. הפונטים מגיעים מ-`@fontsource` ומוגשים מהאתר עצמו (בלי Google Fonts).
